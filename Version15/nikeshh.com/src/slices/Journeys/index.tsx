 ...
 * Props for `Journeys`.
 */
export type JourneysProps = SliceComponentProps<Content.JourneysSlice>;

/**
 * Component for "Journeys" Slices.
 */
const Journeys = async ({ slice }: JourneysProps): Promise<JSX.Element> => {
  const client = createClient();

  const caseStudies = await Promise.all(
    slice.items.map(async (item) => {
      if (isFilled.contentRelationship(item.journey)) {
        return await client.getByID<Content.JourneyDocument>(
          item.journey.id,
        );
      }
    }),
