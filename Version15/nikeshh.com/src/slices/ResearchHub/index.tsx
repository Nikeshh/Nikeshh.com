...
/**
 * Props for `ResearchHub`.
 */
export type ResearchHubProps = SliceComponentProps<Content.ResearchHubSlice>;

/**
 * Component for "ResearchHub" Slices.
 */
const ResearchHub = async ({ slice }: ResearchHubProps): Promise<JSX.Element> => {
  const client = createClient();

  const caseStudies = await Promise.all(
    slice.items.map(async (item) => {
      if (isFilled.contentRelationship(item.research)) {
        return await client.getByID<Content.ResearchDocument>(
          item.research.id,
        );
      }
    }),
